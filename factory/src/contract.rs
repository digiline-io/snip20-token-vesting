...
use crate::state::{
    load, may_load, remove, save, Config, ACTIVE_KEY, BLOCK_SIZE, CONFIG_KEY, INACTIVE_KEY,
    PENDING_KEY, PREFIX_OWNERS_ACTIVE, PREFIX_OWNERS_INACTIVE, PRNG_SEED_KEY,
};
use crate::{rand::sha_256, state::DEFAULT_PAGE_SIZE};

use crate::{
    msg::{
        ContractInfo, FilterTypes, HandleAnswer, HandleMsg, InitMsg, OffspringContractInfo,
        QueryAnswer, QueryMsg, RegisterOffspringInfo, ResponseStatus::Success,
        StoreInactiveOffspringInfo, StoreOffspringInfo,
    },
...
fn try_register_offspring<S: Storage, A: Api, Q: Querier>(
    ...
    let mut my_active_store: CashMap<StoreOffspringInfo, _, _> =
        CashMap::init(owner.to_string().as_bytes(), &mut owners_store);
...
fn try_deactivate_offspring<S: Storage, A: Api, Q: Querier>(
    ...
    let mut inactive_store =
        CashMap::init(owner.to_string().as_bytes(), &mut owners_inactive_store);
...
fn authenticate_offspring<S: ReadonlyStorage>(
    ...
    let info_store: ReadOnlyCashMap<StoreOffspringInfo, _, _> =
        ReadOnlyCashMap::init(ACTIVE_KEY, storage);
...
fn remove_from_persons_active<S: Storage>(
    ...
    let mut load_active: CashMap<StoreOffspringInfo, _, _> =
        CashMap::init(person.to_string().as_bytes(), &mut store);
    ...
    match prefix {
        Some(pref) => {
            ...
            let user_store: ReadOnlyCashMap<StoreOffspringInfo, _> =
                ReadOnlyCashMap::init(key, read);
            Ok(user_store.paging(page_number, size)?)
        }
        None => {
            // get factory's active list
            let active_store: ReadOnlyCashMap<StoreOffspringInfo, _> =
                ReadOnlyCashMap::init(key, storage);
            Ok(active_store.paging(page_number, size)?)
        }
    }
}
...
fn display_inactive_list<S: ReadonlyStorage>(
    ...
    match prefix {
        Some(pref) => {
            ...
            let read = &ReadonlyPrefixedStorage::new(pref, storage);
            let user_store: ReadOnlyCashMap<StoreInactiveOffspringInfo, _> =
                ReadOnlyCashMap::init(key, read);
            list = user_store.paging(page_number, size)?;
        }
        None => {
            // get factory's inactive list
            let active_store: ReadOnlyCashMap<StoreInactiveOffspringInfo, _> =
                ReadOnlyCashMap::init(key, storage);
